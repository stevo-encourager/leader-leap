...
// BACKUP of original promptBuilder.ts created on 2025-01-03
// This backup preserves the previous version before replacing with consolidated prompt
...
export const buildPrompt = (assessmentSummary: AssessmentSummary): string => {
  ...
  // Build available resources list from the resource mappings
  const availableBooks = Object.entries(ALL_RESOURCE_MAPPINGS)
    .filter(([_, resource]) => resource.type === 'book')
    .map(([_, resource]) => `- ${resource.title}`)
    .join('\n');

  const availableFrameworks = Object.entries(ALL_RESOURCE_MAPPINGS)
    .filter(([_, resource]) => resource.type !== 'book')
    .map(([_, resource]) => `- ${resource.title}`)
    .join('\n');

...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Received parameters:', { userId, userEmail, userName });
    ...
    // Get user profile info (only columns that exist)
    ...
    console.log('Profile found, proceeding with email send for user:', userId);

    console.log('Sending simple welcome email...');
    ...
    // Email sent successfully - tracking will be added once we add the columns
    console.log('Email sent, would track with ID:', emailResult.id, 'for user:', userId);
